...
\begin{document}
    ...
    \begin{satz} % Satz 18
        ...
        Sei $U\subseteq\C$ offen, $f: U \to \C$ analytisch, $z_0\in U$ und $R > 0$, sodass $D_R\of{z_0} = \set{\abs{z-z_0} < R}\subseteq U$. Dann existiert eine analytische Funktion $F: D_R\of{z_0} \to \C$ mit $F'\of{z} = f\of{z}~\forall z\in D_R\of{z_0}$. D.h. $f$ hat lokal in der Nähe von $z_0$ eine Stammfunktion.
    ...
    \end{bemerkung}
    \marginnote{[21. Jul]}
    \noindent\textbf{Eindeutigkeit}: Angenommen $\lambda$ und $\mu$ sind 2 Lösungen der folgenden Integralgleichung mit $t_0 < t$:
    \begin{align*}
        \lambda \of{t} &= x_0 + \int_{t_0}^{t} f\of{s, \lambda\of{s}} \dif s\\
        \mu \of{t} &= x_0 + \int_{t_0}^{t} f\of{s, \mu\of{s}} \dif s
        \intertext{in einem Intervall $\interv{t_0 - \alpha, t_0 + \alpha}$. Ziel: $\mu = \lambda$}
        \abs{\lambda\of{t} - \mu\of{t}} &\leq \int_{t_0}^{t} \abs{f\of{s, \lambda\of{s}}- f\of{s, \mu\of{s}}} \dif x
        \intertext{Nach der Lipschitz-Bedingung}
        &\leq L\int_{t_0}^{t} \abs{\lambda\of{s} - \mu\of{s}} \dif s
        \intertext{Definiere}
        w\of{t} &\coloneqq L\abs{\lambda\of{t} - \mu\of{t}} \geq 0\\
        \impl w\of{t} &\leq L\int_{t_0}^{t} w\of{s} \dif s\\
        U\of{t} &\coloneqq \int_{t_0}^{t} w\of{s} \dif s\\
        \impl \dot U\of{t} &\leq L U\of{t}\\
        \equivalent \dot U - LU \leq 0\\
        \frac{\dif}{\dif t}\pair{e^{-L\pair{t-t_0}}U\of{t}} &= e^{-L\pair{t-t_0}}\dot U\of{t} - L e^{-L\pair{t-t_0}}U\of{t}\\
        &= e^{-L\pair{t-t_0}}\pair{\dot U - L U} \leq 0\\
        \impl e^{-L\pair{t-t_0}} U\of{t} &\leq e^{-L\pair{t_0 - t_0}} U\of{t_0} = 0\\
        \impl U\of{t} &\leq 0 \impl U\of{t} = 0\\
        \impl w\of{t} &\leq L U \of{t} \leq 0\\
        \impl w\of{t} &= 0\qed
    \end{align*}
    Sei jetzt $\lambda\of{t_0} = x_0$ und $\mu\of{t_0} = x_1$. Beide lösen $\dot x = f\of{t, x}$:
    \begin{align*}
        \lambda\of{t} &= x_0 + \int_{t_0}^{t} f\of{s, \lambda\of{s}} \dif s\\
        \mu\of{t} &= x_1 + \int_{t_0}^{t} f\of{s, \mu\of{s}} \dif s\\
        \abs{\lambda\of{t} - \mu\of{t}} &= \abs{x_0 - x_1} + \int_{t_0}^{t} \abs{f\of{s, \lambda\of{s}} - f\of{s, \mu\of{s}}} \dif s\\
        w\of{t} &\leq w\of{t_0} + L \underbrace{\int_{t_0}^{t} w\of{s} \dif s}_{\eqqcolon U\of{t}}\\
        \impl \dot U &= w\\
        \impl \dot U\of{t} &= w\of{t} \leq w\of{t_0} + L U\of{t}\\
        \impl \dot U - LU &\leq w\of{t_0}\\
        \frac{\dif}{\dif t}\pair{e^{-Lt}U\of{t}} &= e^{-Lt}\pair{\dot U\of{t} - LU\of{t}}\\
        &\leq e^{-Lt}w\of{t_0}\\
        \impl e^{-Lt}U\of{t}-e^{-Lt_0}U\of{t_0} &\leq w\of{t_0} \int_{t_0}^{t} e^{-Ls} \dif s = \frac{w\of{t_0}}{L}\pair{e^{-Lt_0} - e^{-Lt}}\\
        \impl U\of{t} &\leq \frac{w\of{t_0}}{L}\pair{e^{-L\pair{t-t_0}} - 1}\\
        \impl w\of{t}&\leq w\of{t_0} + L \int_{t_0}^{t} w\of{s} \dif s\\
        &\leq w\of{t_0} + \mu\of{t_0}\pair{-e^{-\pair{t-t_0}} - 1}\\
        &= w\of{t_0}e^{?L\pair{t-t_0}}\qed
    \end{align*}
    sei $D = J \times U$ und $U \subseteq \R^N$ offen. Für Intervall gilt
    \begin{align*}
        \impl \forall t_0 &\in J, t_0\in U\text{ existiert eine lokale Lösung mit}\\
        \dot x &= f\of{t, x}\\
        x\of{t_0} &= x_0\\
        T_t\of{x_0} &= p\of{\xi, ?} \tag{?}\\
        T_-\of{x_0} &= \inf\of{s\in J: ?}\\
        \intertext{$\pair{t_-\of{x_0}, t_+\of{x_0}}$ maximales Intervall für Existenz und Eindeutigkeit. Charakterisierung von $T_+, T_-$}
        T_+ &< \infty\\
        \exists (t_0)_n \in J: t_n n T_+ \text{mit}\\
        \abs{\lambda\of{t_0}} &\to 0
    \end{align*}
    ...
    \marginnote{[22. Jul]}
    \noindent\textbf{Frage}: Wann gilt schon $T_+ = \infty$?\\
    Wir wissen: Ist $T_+ < \infty$, so gilt $(t_n)_n, t_n \nearrow T_+$ mit $\abs{\lambda\of{t_n}} \to \infty$
    \begin{align*}
        \dot \lambda\of{t} &= f\of{t, \lambda\of{t}}\\
        \lambda\of{t_0} &= t_0\\
        \abs{\lambda\of{t}}^2 &= \scalprod{\lambda\of{t}, \lambda\of{t}}
        \intertext{Wobei wir das eukldische Skalarprodukt auf $\R^N$ verwenden}
        \impl \frac{\dif}{\dif t}\abs{\lambda\of{t}}^2 &= \frac{\dif}{\dif t}\scalprod{\lambda\of{t}, \lambda\of{t}}\\
        &= \scalprod{\frac{\dif}{\dif t}\lambda\of{t}, \lambda\of{t}} + \scalprod{\lambda\of{t}, \frac{\dif}{\dif t}\lambda\of{t}}\\
        &= 2 \scalprod{\dot\lambda\of{t}, \lambda\of{t}}\\
        &= 2\scalprod{f\of{t, \lambda\of{t}}, \lambda\of{t}}
        \intertext{$T_+$: Bedingungen}
        \scalprod{f\of{t, x}, x} &\leq C\pair{1 + \abs{x}^2}\\
        \intertext{Für jedes $T > t_0$, $x_0$ existiert $C = C\of{T, x_0} < \infty$, sodass}
        \scalprod{f\of{t, x} , x}&\leq C\of{T, x_0}\pair{1+\abs{x}^2}\quad\forall x\fa t_0\leq t \leq T
    \end{align*}
    Behauptung: $\dsty\limsup_{t\to T} \abs{\lambda\of{t}}^2 < \infty$
    \begin{proof}
        \begin{align*}
            \frac{\dif}{\dif t}\abs{\lambda\of{t}}^2 &= 2 \scalprod{\dot\lambda\of{t}, \lambda\of{t}}\\
            &= 2 \scalprod{f\of{t, \lambda\of{t}}, \lambda\of{t}}\\
            &\leq 2 C \pair{1 + \abs{\lambda\of{t}}^2}\\
            \impl \frac{\dif}{\dif t} \abs{\lambda\of{t}}^2 - 2 C\abs{\lambda\of{t}}^2 &\leq 2C\\
            \frac{\dif}{\dif t}\pair{e^{-2Ct}\abs{\lambda\of{t}}^2} &= e^{-2Ct}\pair{\frac{\dif}{\dif t}\abs{\lambda\of{t}}^2 - 2C\abs{\lambda\of{t}}^2}\\
            &\leq 2C e^{-2Ct}\\
            \impl e^{-2Ct}\abs{\lambda\of{t}}^2 - e^{-2Ct_0}\abs{\lambda\of{t_0}}^2 &\leq e^{-2Ct_0} - e^{-2Ct}\\
            \impl \abs{\lambda\of{t}}^2 &\leq e^{2C\pair{t-t_0}}\abs{\lambda\of{t_0}}^2 + \pair{e^{2C\pair{t-t_0}} - 1}\\
            \impl \limsup_{t\to T} \abs{\lambda\of{t}}^2 &\leq e^{2C\pair{T-t_0}}\abs{\lambda\of{t_0}}^2 + e^{2c\pair{T-t_0}} - 1 < \infty\qedhere
        \end{align*}
    \end{proof}

    \begin{beispiel}[Populationsmodellierung]
        Sei $N\of{t}$ die Anzahl einer Population. Wir hatten schon die logistische Gleichung
        \begin{align*}
            \frac{\dif}{\dif t}N &= aN - bN^2 = aN\pair{1-\frac{b}{a}N}\\
            \dot N &= aN\pair{1-\frac{b}{a}N} = aN\pair{1-\frac{N}{K}}\tag{$K \coloneqq \frac{a}{b}$}\\
            &= aN\pair{\frac{K-N}{K}}
            \intertext{Betrachte zwei unterschiedliche, konkurierende Populationen}
            \dot N_1 &= a_1 N_1\frac{K_1 - N_1 - m_2}{K1}\\
            \dot N_2 &= a_2 N_2 \frac{K_2 - N_2 - m_1}{K2}
            \intertext{Dabei seien $m_1, m_2$ der negative Einfluss der jeweils anderen Population. Wir modellieren dafür}
            m_2 &= \beta_2 N_2\\
            m_1 &= \beta_1 N_1
            \intertext{Betrachte Spezialfall $\beta_1 = \beta_2 = 1$}
            \frac{\dif}{\dif t}\begin{pmatrix}
                                   N_1 \\
                                   N_2
            \end{pmatrix} &= \begin{pmatrix}
                                 a_1 N_1\frac{K_1 - N_1 - m_2}{K1} \\
                                 a_2 N_2 \frac{K_2 - N_2 - m_1}{K2}
            \end{pmatrix} &= \eqqcolon f\of{N_1, N_2}\\
            \intertext{$N_1$ und $N_2$ können nicht negativ werden. Damit kann man sich überlegen, dass für eine Konstante $C$ gilt}
            f\of{N_1, N_2}\begin{pmatrix}
                              N_1 \\
                              N_2
            \end{pmatrix} &\leq C\pair{1 + N_1^2 + N_2^2}
        \end{align*}
        Damit haben wir Existenz der Lösung.
    \end{beispiel}

    \begin{satz}[Konkurenzpopulation]
        Seien $N_1, N_2$ Lösungen der obigen Gleichungen. Ist $K_1 > K_2$, dann gilt
        \begin{align*}
            \begin{pmatrix}
                N_1\of{t} \\
                N_2\of{t}
    ...
    \noindent\textbf{Setting}: Sei $x\of{t}$ die Population der Beute und $y\of{t}$ die Population der Räuber. Ist $y\of{t} = 0$, dann soll $\dot x = ax$. Ist $x\of{t} = 0$, dann soll $\dot y = -cy$. Ist $g\of{t} > 0$, dann fordern wir
    \begin{align*}
        \dot x &= ax - bxy\tag{1.1}\\
        \dot y &= -cy + dxy\tag{1.2}
        \intertext{mit $a,b,c,d > 0$. Berechne die Gleichgewichte $\dot x = 0, \dot y = 0$}
        0 &= ax -bxy = x\pair{a-by}\\
        0 &= -cy+dxy = y\pair{dx - c}\\
        \impl x = 0\quad &\quad y = 0\\
        \intertext{Es ist $x \neq 0$}
        \impl y &= \frac{a}{b}\\
        \impl x &= \frac{c}{d}
        y' = \frac{\dif y}{\dif x} = \frac{\frac{\dif y}{\dif t}}{\frac{\dif x}{\dif t}} = \frac{-cy +dxy}{ax - bxy}\\
        \equivalent y' &= \frac{\dif y}{a x}\\
        5\frac{a-by}{y}\dot y &= \frac{-c + dx}{x}\\
        a\ln y - bx + c\ln x - d x &= ba\\
        \impl \frac{y^a}{e^{-by}}\frac{x^{c}}{e^{dx}} &= K
    \end{align*}

    \begin{lemma}
        Seien $x\of{t}, y\of{t}$ jeweils Lösungen von (1) mit Periode $T > 0$. Dann ist
        \begin{align*}
            \conj{x} &= \frac{1}{T}\int_{0}^{T} x\of{t} \dif t\\
            \conj{y} &= \frac{1}{T}\int_{0}^{T} y\of{t} \dif t\\
            \impl \conj{x} = \frac{c}{d}\quad &\quad \conj{y} = \frac{a}{b}
        \end{align*}

        \begin{proof}
            \begin{align*}
                \dot x &= ax - b xy\\
                \impl \frac{\dot x}{x} &= a-by\\
                \impl \frac{1}{T} \int_{0}^{T} \frac{\dot x}{x} \dif x &= \frac{1}{T}\pair{\ln x\of{T} - \ln x\of{0}} = 0\\
                \impl \frac{1}{T} \int_{0}^{T} \pair{a - by} \dif t &= a - \frac{b}{T}\int_{0}^{T} y\of{t} \dif t\\
                \conj{y} &= \frac{1}{T}\\
                \int_{0}^{T} y'\of{t} \dif t &= \frac{a}{b}\qedhere
            \end{align*}
        \end{proof}
    \end{lemma}

    \subsection{Differentialgleichungen mit Matrizen}

    \marginnote{[29. Jul]} \textbf{1. Fall}: $\dot u = Au$, $u\of{0} = u_0$, $u: I \to\R^n$ mit $A$ als $n\times n$-Matrix. Dann lösen wir mit
    \begin{align*}
        u\of{t}  &= e^{At} u_0
    \end{align*}

    \begin{align*}
        e^{x} &= \sum_{n=0}^{\infty} \frac{x^n}{n!}\tag{$x^0 = 1$}
        \intertext{Sei $A$ eine $n\times n$-Matrix. Dann ist}
        A^2 &= A \cdot A\\
        A^3 &= A^2 \cdot A\\
        A^{n+1} &= A^n \cdot A\\
        A^0 &= \charfunc_{n}\tag{Identität}
        \intertext{Definiere}
        e^{A} &\coloneqq \sum_{n=0}^{\infty} \frac{A^n}{n!} = A^0 + A + \frac{A^2}{2} + \frac{A^3}{3!} + \ldots
    \end{align*}
    Es gilt
    \begin{align*}
        \norm{A} &= \norm{A}_{op} \coloneqq \sup_{\norm{u} = 1}\norm{A u}\\
        \impl \norm{AB} &\leq \norm{A}\norm{B}\\
        \impl \norm{A^n} &\leq \norm{A}^n\\
        \impl \norm{ABu} = \norm{Aw} &\leq \norm{A}\norm{Bu}\tag{$w\coloneqq Bu$}\\
        \sum_{n=0}^{\infty} \frac{\norm{A^n}}{n!} &\leq \sum_{n=0}^{\infty} \frac{\norm{A}^n}{n!} = \exp\of{\norm{A}} < \infty\\
        \intertext{Das heißt wir haben Konvergenz. Außerdem ist}
        \frac{\dif}{\dif t}e^{At} &= \frac{\dif}{\dif t}\sum_{n=0}^{\infty} \frac{(At)^n}{n!}\\
        \impl Ae^{At} &= \sum_{n=0}^{\infty} \frac{\dif}{\dif t} \frac{A^n}{n!} t^n = \sum_{n=1}^{\infty} \frac{A^n}{(n-1)!}t^{n-1}\\
        &= A \sum_{n=1}^{\infty} \frac{A^{n-1}}{(n-1)!}t^{n-1} = A \exp\of{At}
        \intertext{Das heißt $\dot u = A u$ hat eine Lösung}
        \impl u\of{t} &= e^{At} u_0
    \end{align*}
    Wie berechnen wir $\exp\of{A}$? Wenn $A$ symmetrisch ist, dann gilt
    \begin{align*}
        \exists T\in\R^{n\times n}\text{ invertierbar}\colon A &= T\text{Diag}\of{\lambda_1, \ldots, \lambda_n}T^{-1}\\
        \impl e^{At} &= T e^{A}T^{-1} = T \text{Diag}\of{e^{\lambda_1}, \ldots, e^{\lambda_n}} T^{-1}
    \end{align*}
    Eigenschaften: \textit{(fehlen)}\\~\\
    \noindent Sei $N\in\mL\of{\R^n, \R^n}$ nilpotent. Das heißt es gibt ein $n_0\in\N$, sodass $N^{n_0} = 0$. Dann ist
    \begin{align*}
        \exp\of{Nt} &= \charfunc + \sum_{n=0}^{\infty} \frac{N^n}{n!}t^n = \charfunc + \sum_{n=0}^{n_0 - 1} \frac{N^n}{n!}t^n
    \end{align*}\\~\\
    \noindent Seien $A, B\in\mL\of{\R^n, \R^n}$, sodass $A$ und $B$ kommutieren. Das heißt $AB = BA$. Dann kann man nachrechnen, dass
    \begin{align*}
        e^{(A+B)t} &= e^{At}e^{Bt}
    \end{align*}
    Das heißt, wenn wir $e^{Ct}$ berechnen wollen und sich $C = A + B$ durch zwei kommutierende Matrizen $A, B$ darstellen lässt, dann können wir die Matrixexponentiale einzeln berechnen.

    \begin{beispiel}
        $A$ habe Eigenwert $\lambda$ und habe Jordan-Block
        \begin{align*}
            \exists v\neq 0\colon \pair{A-\lambda}v &\neq 0,~\pair{A-\lambda}^2 = 0
        \end{align*}
        Sei außerdem $u$ der Eigenvektor zum Eigenwert $\lambda$, das heißt $Au = \lambda u$. Es ist
        \begin{align*}
            \exp\of{At} &= \exp\of{\pair{A-\lambda}t + \lambda t} = \exp\of{\lambda}\exp\of{\pair{A-\lambda}t}\\
            \exp\of{At}u &= e^{\lambda t}\exp\of{\pair{A - \lambda}t}u = e^{\lambda t}u\\
            \exp\of{At}v &= e^{\lambda t}\exp\of{\pair{A-\lambda}u??}v = e^{\lambda t}\pair{\pair{A - \lambda}vt + v}\\
        \end{align*}
        Bilder dazu: Bernd Aulbach, Seite 197
    \end{beispiel}

    Sei $\lambda$ Eigenwert in $A$ mit $\lambda = \alpha + i \beta$. Dann ist $\conj{\lambda} = \alpha - i\beta$ ein EW von $A$. Eigenvektor $u_0\in\C^n$ von $A$ bzgl. $\lambda$. Dann ist $u_0 = v_1 + i v_2$ mit $v_i\in\R^n$. Es ist
    \begin{align*}
        e^{At}u_0 &= e^{\pair{\alpha + i\beta}t}u_0\\
        &= e^{\alpha t}e^{i\beta t}\pair{v_1 + i v_2}\\
        &= e^{\alpha t}\pair{\cos \beta t + i \sin \beta t}\pair{v_1 + i v_2}\\
        &= e^{\alpha t}\pair{\cos \beta t v_1 - \sin \beta t v_2} + ie^{\alpha t}\pair{\cos \beta t v_2 + \sin \beta t v_1}\eqqcolon \psi\of{t}
    \end{align*}
    $\phi\of{t}$ ist eine komplexe Funktion, die eine reelle Differentialgleichung löst. Dann löst aber auch $\conj{\phi\of{t}}$ die Gleichung und damit auch $\phi\of{t} + \conj{\phi\of{t}}$, was eine reelle Funktion ist. Das heißt wir kriegen
    \begin{align*}
        e^{\lambda t} \cdot\text{Streckung oder Drehung} + \text{polynomielle Faktoren (Jordan-Block)}
    \end{align*}

    (??)
    \begin{align*}
        \dot x &= f\of{x},~ f\of{x_{\ast}} = 0\\
        f\of{x_{\ast}} &= Au + g\of{u}\\
        \impl\dot u &= A u + g\of{u}\\
        \dot u &= Au + h\of{t}\\
        u\of{0} &= u_0
        \intertext{Ansatz $u\of{t} = \exp\of{\lambda t}v\of{t}$}
        \dot u &= A e^{At} v + e^{At}\dot v\\
        \impl Au + h &= Ae^{At}v + h\\
        \impl e^{At}\dot v &= h
        \dot v\of{t} &= e^{-At}h\of{t}\\
        v\of{t} &= \int_{0}^{t} e^{-As}h\of{s} \dif s\\
        \impl u\of{t} &= e^{At}v = e^{At} \int_{0}^{t} e^{-As}h\of{s} \dif s\\
        &= \int_{0}^{t} e^{A\pair{t-s}}h\of{s} \dif s\\
        \impl u\of{t} &= e^{At}u_0 + \int_{0}^{t} e^{A\pair{t-s}}h\of{s} \dif s\tag{Variation der Konstanten}\\
        h\of{s} &= g\of{u\of{s}}\\
        \impl u\of{t} &= e^{At}u_0 + \int_{0}^{t} e^{A\pair{t-s}}g\of{u\of{s}} \dif s
    \end{align*}
    (??)
